class PokerHand
  ...
  def four_of_a_kind?
    if (md = (by_face =~ /(.). \1. \1. \1./))
      ...
      result << Card.face_value(Regexp.last_match(1)) if (md.pre_match + md.post_match) =~ /(\S)/
  ...
  def straight?
    if hand.size >= 5
      ...
      i = 0
      until transform.match(/^\S{3}( [1-9x]\S\S)+( 0\S\S)*$/) || (i >= hand.size)
  ...
  def three_of_a_kind?
    if (md = (by_face =~ /(.). \1. \1./))
      ...
      if matches
        ...
        result << Card.face_value(Regexp.last_match(1)) if matches
        matches = arranged_hand.match(/(?:\S\S ){3}(\S)\S (\S)/)
        result << Card.face_value(Regexp.last_match(2)) if matches
  ...
  def two_pair?
    ...
    if (md = (by_face =~ /(.). \1.(.*?) (.). \3./))
      ...
      if matches
        ...
        result << Card.face_value(Regexp.last_match(1)) if matches # face value of the kicker
        return [result, arranged_hand]
      end
    end
    false
  ...
  def <=>(other_hand)
    score[0].compact <=> other_hand.score[0].compact
  end
  ...
  def <<(new_cards)
    new_cards = [new_cards] if new_cards.is_a?(Card) || new_cards.is_a?(String)
  ...
  RESOLVING_METHODS = [:each, :size, :-].freeze
  ...
  def match?(expression)
    ...
    return expression.any? { |e| match?(e) } if expression.is_a? Array

    faces = @hand.map(&:face).sort.reverse
    suited = @hand.map(&:suit).uniq.size == 1
    if expression =~ /^(.)(.)(s|o|)(\+|)$/
      face1 = Card.face_value(Regexp.last_match(1))
      face2 = Card.face_value(Regexp.last_match(2))
      raise ArgumentError, "Invalid expression: #{expression.inspect}" unless face1 && face2
      suit_match = Regexp.last_match(3)
      plus = (Regexp.last_match(4) != '')

      if plus
        if face1 == face2
          face_match = ((faces.first == faces.last) && (faces.first >= face1))
        elsif face1 > face2
          face_match = ((faces.first == face1) && (faces.last >= face2))
        else
          face_match = (((faces.first - faces.last) == (face2 - face1)) && (faces.last >= face1))
        end
      else
