...
class TestPokerHand < Test::Unit::TestCase
  context 'A PokerHand instance' do
    setup do
      ...
      @full_boat = PokerHand.new(%w(2H 2D 4C 4D 4S))
    ...
    should 'return card sorted by face value' do
      assert_equal([13, 13, 13, 8, 1], @trips.by_face.hand.collect(&:face))
    end

    should 'return cards sorted by suit' do
      assert_equal([3, 2, 1, 0, 0], @trips.by_suit.hand.collect(&:suit))
    end
